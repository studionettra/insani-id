                                ...
                                <div className="bg-amber-50 rounded-2xl p-5 border border-amber-200 text-sm space-y-4">
                                    ...
                                    <ul className="space-y-3">
                                        {(bankAccounts && bankAccounts.length > 0 ? bankAccounts : [
                                            { id: 1, bank_name: 'Bank Syariah Indonesia (BSI)', account_number: '713 219 5026', account_name: 'A.n Insani Indonesia' },
                                            { id: 2, bank_name: 'Bank Rakyat Indonesia (BRI)', account_number: '0345 0100 1366 304', account_name: 'A.n Insani Indonesia' },
                                        ]).map((acc: any) => (
                                            <li key={acc.id} className="flex justify-between items-center bg-white p-3.5 rounded-xl border border-amber-200/80 shadow-xs">
                                                <div>
                                                    <span className="block font-bold text-slate-800">{acc.bank_name}</span>
                                                    <span className="text-slate-700 font-mono text-lg font-bold tracking-wider">{acc.account_number}</span>
                                                    <span className="block text-xs text-slate-500">{acc.account_name}</span>
                                                </div>
                                                <button 
                                                    onClick={() => copyToClipboard(String(acc.account_number).replace(/\s+/g, ''))} 
                                    ...
                                    <div>
                                        {(() => {
                                            ...
                                            const message = `Halo Admin Insani, saya sudah mentransfer donasi sebesar ${formatCurrency(Number(donation.amount))} untuk ID Transaksi ${donation.donation_code} pada program ${title}. Mohon dicek dan diverifikasi, terima kasih!`;
                                            return (
                                                <a 
